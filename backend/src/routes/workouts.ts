...
workoutRoute.get("/:id", async (c) => {
  ...
  if (workout.user_id !== user.id) return c.json({ error: "Unauthorized" }, 403);
  ...
  try {
    const info = await run(
      `INSERT INTO workouts (user_id, name, calories_burned, duration_minutes, workout_date, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [user.id, name, calories_burned, duration_minutes || 0, workoutDate, createdAt]
    );
...
workoutRoute.delete("/:id", async (c) => {
  ...
  const workout = await get<{ user_id: number }>("SELECT user_id FROM workouts WHERE id = ?", [id]);
...
workoutRoute.put("/:id", async (c) => {
  ...
  await run("UPDATE workouts SET name = ?, calories_burned = ?, duration_minutes = ? WHERE id = ?", 
    [name, calories_burned, duration_minutes || 0, id]);

  // Recalculate badges/challenges after workout adjustments
  const badgeUnlocks = await checkBadges(user.id);
  const challengeUpdates = await checkChallenges(user.id);
  return c.json({ success: true, unlocks: badgeUnlocks, challengeUpdates });
});
