...
workoutRoute.post("/", async (c) => {
    ...
    try {
        const info = db
            .prepare(`INSERT INTO workouts (user_id, name, calories_burned, duration_minutes, workout_date, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
...
workoutRoute.put("/:id", async (c) => {
    ...
    const workout = db.prepare("SELECT user_id FROM workouts WHERE id = ?").get(id);
    if (!workout)
        return c.json({ error: "Workout not found" }, 404);
    if (workout.user_id !== user.id) {
        return c.json({ error: "Unauthorized" }, 403);
