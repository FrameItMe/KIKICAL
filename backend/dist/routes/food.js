...
foodRoute.get("/search", async (c) => {
    ...
    const foods = await all(`SELECT * FROM food
     ...
     ORDER BY
       ...
       END,
       LENGTH(name) ASC
     LIMIT 30`, [contains, qLower, prefix]);
    return c.json(foods);
});
...
foodRoute.get("/", async (c) => {
    const foods = await all("SELECT * FROM food LIMIT 50", []);
    return c.json(foods);
});
// GET /food/:id - Get single food
foodRoute.get("/:id", async (c) => {
    const id = c.req.param("id");
    const food = await get("SELECT * FROM food WHERE id = ?", [id]);
    if (!food)
...
foodRoute.post("/", async (c) => {
    ...
    const info = await run(`INSERT INTO food (name, category, calories_per_100g, protein_per_100g, carb_per_100g, fat_per_100g, default_serving_grams, created_by_user)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
        name,
        category || "General",
        calories_per_100g,
...
foodRoute.put("/:id", async (c) => {
    ...
    const id = c.req.param("id");
    const body = await c.req.json();
    const food = await get("SELECT created_by_user FROM food WHERE id = ?", [id]);
    if (!food)
        return c.json({ error: "Food not found" }, 404);
    ...
    const { name, category, calories_per_100g, protein_per_100g, carb_per_100g, fat_per_100g, default_serving_grams, } = body;
    await run(`UPDATE food SET name=?, category=?, calories_per_100g=?, protein_per_100g=?, carb_per_100g=?, fat_per_100g=?, default_serving_grams=? WHERE id=?`, [
        name,
        category,
        calories_per_100g,
        protein_per_100g,
