...
mealRoute.get("/:userId", (c) => {
    ...
    const meals = db
        .prepare(`SELECT m.*, f.name as food_name, f.calories_per_100g, f.protein_per_100g, f.carb_per_100g, f.fat_per_100g 
       FROM meal_log m 
...
mealRoute.post("/", async (c) => {
    ...
    try {
        const info = db
            .prepare(`INSERT INTO meal_log (user_id, food_id, meal_date, meal_type, portion_multiplier, logged_at)
         VALUES (?, ?, ?, ?, ?, ?)`)
...
mealRoute.delete("/:id", (c) => {
    ...
    const log = db.prepare("SELECT user_id FROM meal_log WHERE id = ?").get(id);
    if (!log)
