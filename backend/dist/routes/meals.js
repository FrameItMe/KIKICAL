...
mealRoute.get("/:userId", async (c) => {
    ...
    const meals = await all(`SELECT m.id, m.user_id, m.food_id, m.meal_date, m.meal_type, m.portion_multiplier, m.logged_at,
     ...
     ORDER BY m.logged_at DESC`, [user.id, date]);
    return c.json(meals);
});
...
mealRoute.post("/", async (c) => {
    ...
    try {
        const info = await run(`INSERT INTO meal_log (user_id, food_id, meal_date, meal_type, portion_multiplier, logged_at)
       VALUES (?, ?, ?, ?, ?, ?)`, [user.id, food_id, meal_date, meal_type, portion_multiplier, loggedAt]);
...
mealRoute.delete("/:id", async (c) => {
    ...
    const log = await get("SELECT user_id FROM meal_log WHERE id = ?", [id]);
    if (!log)
...
mealRoute.put("/:id", async (c) => {
    ...
    const id = c.req.param("id");
    const body = await c.req.json();
    const { meal_type, portion_multiplier, food_id, update_food, name, calories_per_100g, protein_per_100g, carb_per_100g, fat_per_100g } = body;
    if (!meal_type || portion_multiplier === undefined) {
        return c.json({ error: "Missing required fields (meal_type, portion_multiplier)" }, 400);
    }
    const log = await get("SELECT user_id, food_id FROM meal_log WHERE id = ?", [id]);
    if (!log)
        return c.json({ error: "Log not found" }, 404);
    if (log.user_id !== user.id) {
        return c.json({ error: "Unauthorized" }, 403);
    ...
    if (update_food) {
        ...
        const currentFood = await get("SELECT id, category FROM food WHERE id = ?", [log.food_id]);
        if (currentFood && currentFood.category === "Custom") {
            await run(`UPDATE food
         SET name = ?, calories_per_100g = ?, protein_per_100g = ?, carb_per_100g = ?, fat_per_100g = ?
         WHERE id = ?`, [name, calories_per_100g, protein_per_100g, carb_per_100g, fat_per_100g, currentFood.id]);
            nextFoodId = currentFood.id;
        }
        else {
            const info = await run(`INSERT INTO food (name, category, calories_per_100g, protein_per_100g, carb_per_100g, fat_per_100g, default_serving_grams)
         VALUES (?, 'Custom', ?, ?, ?, ?, 100)`, [name, calories_per_100g, protein_per_100g, carb_per_100g, fat_per_100g]);
    ...
    await run("UPDATE meal_log SET meal_type = ?, portion_multiplier = ?, food_id = ? WHERE id = ?", [meal_type, portion_multiplier, nextFoodId, id]);
    return c.json({ success: true });
});
