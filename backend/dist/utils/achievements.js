 ...
 */
export function checkChallenges(userId) {
    const today = new Date().toLocaleDateString("sv-SE");
    const completions = [];
...
function awardBadge(userId, badgeName) {
    ...
    if (!existing) {
        const today = new Date().toLocaleDateString("sv-SE");
        db.prepare("INSERT INTO user_earned_badges (user_id, badge_id, earned_date) VALUES (?, ?, ?)").run(userId, badge.id, today);
        return {
            name: badge.name,
...
function updateChallengeProgress(userId, challengeName, currentValue) {
    const challenge = db
    ...
    if (existing) {
        ...
        db.prepare("UPDATE user_challenge_progress SET current_value = ?, status = ? WHERE id = ?").run(currentValue, status, existing.id);
    }
    else {
        justCompleted = status === "completed";
        db.prepare("INSERT INTO user_challenge_progress (user_id, challenge_id, current_value, status, start_date) VALUES (?, ?, ?, ?, ?)").run(userId, challenge.id, currentValue, status, today);
    }
    if (justCompleted) {
        return {
            name: challengeName,
...
export function calculateMealStreak(userId) {
    const dates = db
...
function countCalorieGoalDays(userId) {
    const targets = db
    ...
    if (!targets)
        return 0;
    const meals = db
        .prepare(`SELECT meal_date, SUM(f.calories_per_100g * m.portion_multiplier) as total
       FROM meal_log m
...
function countProteinTargetDays(userId) {
    const targets = db
    ...
    if (!targets)
        return 0;
    const meals = db
        .prepare(`SELECT meal_date, SUM(f.protein_per_100g * m.portion_multiplier) as total
       FROM meal_log m
       JOIN food f ON m.food_id = f.id
       WHERE m.user_id = ?
       GROUP BY meal_date`)
...
function countMacroTargetDays(userId) {
    const targets = db
        .prepare("SELECT daily_protein_target, daily_carb_target, daily_fat_target FROM targets WHERE user_id = ?")
        .get(userId);
    if (!targets)
        return 0;
    const meals = db
        .prepare(`SELECT meal_date,
